...
class AddrValidator:
    ...
    def __init__(
        ...
        self.request_blacklist = request_blacklist or set()
        self.request_whitelist = request_whitelist or set()
    ...
    def _requests_matches_pattern(self, method, hostname, path, pattern):
        """checks whether the request method and url matches the pattern"""

        if isinstance(pattern, str):
            return self._hostname_matches_pattern(hostname, pattern)
        elif len(pattern) == 3:
            (hostname_regex, m, path_regex) = pattern
            if self._hostname_matches_pattern(hostname, hostname_regex):
                if method == m.upper() and re.fullmatch(path_regex, path):
                    return True
            return False
        else:
            raise ConfigException(f"invalid pattern: {pattern}")
    ...
    def is_path_method_allowed(self, url, method):
        # parse url
        ...
        for pattern in self.request_whitelist:
            if self._requests_matches_pattern(method, hostname, path, pattern):
                return True

        for pattern in self.request_blacklist:
            if self._requests_matches_pattern(method, hostname, path, pattern):
                return False
        return True
