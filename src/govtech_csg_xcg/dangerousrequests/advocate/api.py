...
import govtech_csg_xcg.dangerousrequests.advocate

from .adapters import ValidatingHTTPAdapter
from .addrvalidator import AddrValidator
from .exceptions import MountDisabledException, UnacceptableRequestException
...
class Session(RequestsSession):
    ...
    DEFAULT_VALIDATOR = AddrValidator()
    """
    ...
    def request(self, method, url, **kwargs):
        """Wrapper around `request()` to check method and urls"""
        # do some checks on the method and url

        if not self.validator.is_path_method_allowed(url, method):
...
class RequestsAPIWrapper:
    ...
    def __getattr__(self, item):
        ...
        try:
            ...
            return getattr(govtech_csg_xcg.dangerousrequests.advocate, item)
